...
def convergenceAnalysis(self, analyticalValue,nEdgeNodes, elemTypes ):
    ...
    for n in nEdgeNodes:
        for elemType in elemTypes:
            ...
            generateMesh(self, showGmshMesh=False, elementType='QUAD', nEdgeNodes=n, order=myOrder)
            solveModel(self, resultsScaleIntForces = (1, 1), resultsScaleVertDisp = 1e3/1000**4, elemType=elemType, internalForcePosition = 'center')
            outRes[k,1] = np.abs(patchTestModel.results.wMax[2]-analyticalValue)
            outRes[k,0] = 1/n
            k+=1
...
# generateMesh(patchTestModel, showGmshMesh=False, elementType='QUAD', nEdgeNodes=7, order='linear')
...
# elemTypes = ['L-R', 'MITC4-N', 'Q-R']
# solveModel(patchTestModel, resultsScaleIntForces = (1, 1), resultsScaleVertDisp = 1/a**4*1000, elemType=elemTypes[0], internalForcePosition = 'center')
# # # print('MITC')
...
markerSymbols = ['s', '^']
ax=plt.axes()
outRes = convergenceAnalysis(patchTestModel,-analyticalValue, nEdgeNodes = [3, 5, 9, 17, 33], elemTypes = elemTypes )
